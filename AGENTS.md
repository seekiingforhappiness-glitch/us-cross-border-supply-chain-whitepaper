...
模型分工（2026-07-07 人指定；2026-07-13 Daniel 升级为三层，为 token 效率）：
**Fable=编排层**（规划、评审、验收、裁决对话、架构判断——不亲自写大块代码）；
**Opus 4.8=复杂执行层**（大切片开发/多文件重构/强推理实现）；
**Sonnet 5=常规执行层**（调研检索/文档整理/小改动/机械任务）。
派发子代理必须显式指定 model 参数，禁止默认继承主会话模型（Fable 跑执行=烧钱）。
（角色与模型解绑，任何接管编排模型继承此职责。）子代理产出必须通过全部评估器 +
主会话审查后才可提交；子代理不得 git commit、不得修改 plan/决策日志、
遇规格歧义必须在报告中列明而非自行裁决。

与 Daniel 协作的软知识（跨会话/跨模型交接必读）：
- Daniel 是跨境供应链学习者非工程师：需要他裁决时**把规格决策翻译成业务场景问题**
  （参考先例：C1 问的是"改期后再延误要不要再报警"，不是"rescheduled 是否终态"）
- 工程取舍可请求打包授权（先例：C3-C6、E/F 系列整批批准），业务语义决策必须他亲自答
- **健壮性/UX 常任授权（V22，2026-07-19）**："明显提升健壮性和用户体验的，自己决定就好了"
  ——不触碰业务语义红线（对象/规则/KPI/权限边界/冻结区/真值）的此类改进可自行裁决执行，
  事后 STATUS 报告即可；触红线或两可业务取舍仍走 §3
- 他的验收方式是亲手走查 UI + 对断言打勾；给他的操作指引要具体到"点哪个按钮、切哪个角色"
- 评审纪律：子代理报告一律重跑复核，不信报告只信输出（先例：抓过报告金额笔误）
- 文化：预测错了记勘误（H1）、KPI 失败记失败（XE3）、指标不达标禁止改指标——诚实优先
...
### 6.1 执行者纪律（V16③ 2026-07-16 Daniel 批准入宪，源=Monday 施工手册吸收，docs/research/2026-07-16-monday-deep-research-absorb.md §3.3）

1. **停止条件的反面**：普通编译错误、测试失败、实现困难、或"我觉得有更优架构"**都不是**停止理由——应在本工作单范围内自行修复；停止只为：规格冲突、触及冻结区/真值/基线红线、需要新的业务裁决、发现不可逆风险。
2. 失败路径、越权路径、幂等重试路径、并发冲突路径，按适用范围**有测试**才算完成。
3. 迁移必须在**空库与上一版本库**都跑通；只前向修复，不做破坏性回滚。
4. **反作弊**：不吞错、不写假实现、不写永远通过的 stub、不跳过校验。
5. 交付报告只写**真实跑过的命令与真实输出**；没跑过/失败的如实标注。
6. **硬边界（凌驾以上五条的读法）**：以上是执行者的自我检查单，**绝不替代主会话独立复核门**——子代理"自填报告=完成"不成立，产出必须过全部评估器 + 主会话重跑复核（双层结构，源自全局规则 6：生成与验证物理隔离）。
