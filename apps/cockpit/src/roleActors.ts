...
// V23②（Daniel 批：合规获协调发起/跟进权，缘起轮3合规陌生人"发现问题后系统内无处置入口"）
// compliance 同样接入协调写权限；sales 与协调权限组（COORD_PERMS）仍无关，驾驶舱内保持只读。
//
...
export interface RoleMeta {
   ...
   * （app/actions.py 逐行核对确认）。sales 本批仍在驾驶舱只读（不在 COORD_PERMS），其 actor 值只用于
   * 展示文案、从不进入写请求。compliance 自 V23② 起接入 COORD_PERMS（协调写权限），其 actor 值
   * （u-compliance-us）现在**会**真实进入协调写请求的 X-Actor 头（审计留痕『谁做的』）——但
   * DEMO_ROSTER 仍无对应条目：这是按既有 `u-<角色码>-us` 命名惯例合成的占位符，**无 DEMO_ROSTER
   * 背书**。侦察确认这不阻塞功能：coordination_actions 的 actor 字段本身是自由字符串（只落
   * action_log 审计 / execute_command 幂等键，不查 DEMO_ROSTER）；唯一受影响面是
   * app/data_scope.py resolve_actor() 的 region 派生，其"无匹配 → (None, DEFAULT_DEMO_REGION)"
   * 容错早已文档化，只影响视图粒度不影响写操作成功与否。若未来要给 compliance 一个有 DEMO_ROSTER
   * 背书的身份（如需要精确 region 归属），须先补 Owner 条目——本批未做（超出 COORD_PERMS 扩权范围）。
   */
  actor: string;
}
...
export const ROLES: RoleMeta[] = [
  ...
  // 但不在 _COST_VISIBLE，钱区/成本情景仍掩码。compliance 自 V23② 起在 COORD_PERMS 内，本批同步
  // 接入协调流写操作（催办/记回应/升级/达成/谈崩 + 发起协调），与 cs/procurement 同款。
