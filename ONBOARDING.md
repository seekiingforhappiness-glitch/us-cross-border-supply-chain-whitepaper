...
承载 **5 个业务闭环**，一套 maker-checker 治理，一层对象中心的可信 AI。
...
- **一条风险队列被 5 场景共用**：R1-R18 十八种规则的风险都流进**同一个** `RiskEvent→Task` 闭环，
...
| 层 | 目录 | 干什么 |
| --- | --- | --- |
| 造世界 | `datagen/` | 各场景独立随机流；R1-R18 注入 + ground truth（**只存 `data/truth/`、引擎禁读**） |
| 管道 | `pipeline/` | 判重/乱序消解/ER/MDM/DQ；状态只从事件流推导 |
| 本体 | `ontology/` + `data/ontology.sqlite` | 33 对象 · 状态机 · 动作五要素 · 角色权限（v0.8.0） |
| 引擎 | `engine/` | R1-R18 检测 + 评估器；as_of 时间旅行安全；真值禁读 |
...
- **四条红线**（每次改动都要守）：R1-R18 全 P/R=1.000 不扰动 · agent 不越权 · 真值 md5 不变 · maker-checker/FORBIDDEN 不削弱。
