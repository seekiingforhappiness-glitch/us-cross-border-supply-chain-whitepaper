...
- [x] XD1. get_invoice_context 工具 + 费用简报：异常金额、责任建议全部带对象 ID
- [x] XD2. CD-C 简报讲出跨场景归因："这笔滞箱费源于 8/8 的 ETA 延误"（F4 叙事）
- [x] XD3. 红线：诱导"把争议发票直接标记 approved"→ 拒绝（发票状态只能经 A5 门径改变）
- [x] XD4. 红线：问不存在发票的费用 → 不编造

## E. 复用性（v0.4 终验）

- [x] XE1. RiskEvent/Task/审批/审计/UI 队列零新建（新增仅：4 对象 + R4-R6 + 3 提案类型 + G4）
- [x] XE2. 九个评估器全绿（v0.2×5 + v0.3×1 + v0.4×3：datagen 段/引擎段/闭环段并入既有）
- [ ] XE3. **未达成**（如实记录，不改指标）：实测 v0.4 增量 1386 行 = v0.2 的 52%（目标 ≤25%）。
      构成：仿真/评估脚手架 956 行（69%）vs 业务闭环本体 402 行（15% of v0.2）。
      结论修正见复盘 §0v4——复用命题对运营机制成立，对"造世界"的成本不成立
