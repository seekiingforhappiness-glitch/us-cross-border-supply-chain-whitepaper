...
cs/procurement/compliance/sales；数据范围/脱敏走 API X-Role 同源，前端只透传不复制规则，
scope parity 测试同步扩展）②**协调催办进驾驶舱**（CL1 六动作经 Command 总线暴露人类 HTTP
通道，权限=COORD_PERMS 同集，审计/幂等/世界跟随不变量照旧）③**审批理由必填**（批准与驳回
均必填非空理由，落审计与处置记忆场外依据，maker-checker 不变）④**演示默认世界改模拟世界**
（纯前端初始态；验证世界仍是评估基线世界不动）⑤**首屏优先级**（形态由执行会话定夺——首选
"今日焦点"条现算 top3 而非重排七卡，卡位稳定性保留）。
**打包授权（软知识升级）**：明显提升健壮性与用户体验、不触碰业务语义红线（对象/规则/KPI/
权限边界/冻结区/真值）的改进，执行会话可自行裁决并执行，事后在 STATUS 报告即可，无需逐项
请批。触碰红线或有两可业务取舍的仍走 §3 变更协议。

**V21 — L-UX 循环遗留三裁决（2026-07-19，Daniel 原话"1.可以；2.同意；3.OK"）。**
①**自队金额可见**：指派给某团队的待批提案，其金额对该团队角色可见（ops 可见 assignee_role=ops
行的金额）——权限颗粒度放宽须**三面同源**落地（驾驶舱聚合脱敏+对象读端点+AI 工具面共用同一
例外规则），不许驾驶舱单点开洞造成口径漂移；其余角色与非自队行为脱敏照旧。②**证据空缺提示**：
证据链四块全无数据的提案，证据卡加"四项证据均无数据，建议线下复核后再拍板"琥珀提示（不设
金额门槛，全空即提示）。③键盘等价列表由 Daniel 真机自验（测试面板按键管道哑，判决实验在案）。
...
（project44/Shippeo 正当用法）；且抽象双列丢失"跨太平洋 vs 跨大西洋方向差异"的地理信息量。
履约视图升级为夜景真地图（内嵌简化海岸线静态数据非依赖、港口真实经纬度投影、航线弧、
异常色、下钻不变），走廊图退役。②**AI 工作流故事卡化**：同一风险/任务链的检测→提案→审批
事件折叠为一张故事卡，主句人话模板（"发现重复付款 $7,157→AI 已提案追回，候财务审批"），
[sim] 文本前缀清除（SIM 徽标已表达）、动作码/规则码/severity 全部人话化、裸 ID 收进详情——
Streamlit ux_copy 清洗层精神在驾驶舱的补课。处置单 B5。

**V10 — 驾驶舱首屏形态定案：候选 C（2026-07-14，Daniel 亲批"C"）。**
基于 Sonnet 12 厂商调研（docs/research/2026-07-14-supply-chain-cockpit-best-practices.md）的
三候选中选定 **C=七区指挥墙 + 履约卡 tile 内切换"航线走廊图"**（SAP IBP 验证过的交互范式）：
默认首屏=七张区卡片指挥墙（大数字+趋势+告警+区摘要，告警卡辉光排前），下钻四段式（总览→
区队列→详情→动作）；履约卡右上角显眼切换钮→整体展开航线走廊图（左中国港列/右美国港仓列/
弧线束粗细=在途票柜数、颜色=异常状态，点弧线→该航线异常队列）。等距分层图自驾驶舱移除、
归档为透视镜 v3 素材；AI 工作流/影响面板/对象卡/图标/质感 token 保留。B 记分卡与 A 地图中心
两候选未选（A 明确不推荐：六域无地理坐标+航线单一）。

**V9 二补 — 驾驶舱与透视镜形态混淆批评（2026-07-14，Daniel 看 B3 等距版原话：
"1.可读性极差；2.中间全景图这个你跟后台的透视镜搞混了，这个你调研下（用sonnet模型）最佳实践，
参考下供应链的驾驶舱一般长什么样。然后再给出方案"）。** 诊断确认：等距分层对象图=系统结构
可视化（客户一层/订单一层/在途一层——数据库视角），属后台透视镜语言；驾驶舱中央应是**业务
视图**（供应链地理/流向/网络本身）。V4⑤"Palantir 等距图"锚点被误用于驾驶舱首屏。
处置：①按 Daniel 指定派 Sonnet 调研主流供应链控制塔/驾驶舱产品首屏形态（一手来源，证据分级）
②基于调研出画面三稿方案候裁，**方案获批前不动代码** ③B3 的等距分层图资产不弃——它是透视镜
v3 的正确素材（届时迁移）。

**V9 补记 — 影响走廊可视化形态批评（2026-07-14，Daniel 贴 B2 走廊点亮态截图，原话：
"这看起来很乱，要结合实际情况，用合适的可视化图片来展现"）。** 诊断：实体级线条爆炸是形态
错配——"一风险对多订单行"的传播信息在实体多时必乱，问题不在线的画法在可视化形式选择。
处置（已注入进行中的 B3 单）：①全景侧只做聚合级高亮（受影响组块整块点亮+计数徽标，≤5 条
聚合主干光路，粗细=数量/金额），实体级线条网作废；②同步滑出结构化"影响分析面板"（风险摘要→
受影响行段→波及客户段→AI 提案入口，紧凑表格）。**方法论入账：每类信息用最适形态——分布→
条形、构成→分桶、流向→聚合主干、明细→表格；图形元素超约 30-50 个可读阈值必须聚合或转面板，
绝不硬画。**
