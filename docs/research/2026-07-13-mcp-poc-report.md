...
**状态**：**部分通**（server 侧全通过；claude CLI 侧因未登录卡在最后一步，登录后一条命令可验完）
...
| **订阅通道端到端真调用** | **未验完（阻塞在登录）** | `claude -p` 在 21ms 内返回 `Not logged in`，**尚未 spawn MCP server**（call_log 无新增） |

**总判定：部分通。** server 与协议层已证明可行且可留证；订阅通道的"最后一米"卡在 CLI 未登录，非机制问题。**严禁**把这记成"通"——端到端真调用尚未发生。
